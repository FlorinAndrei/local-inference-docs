...
These are faster than a MacBook Pro (more bandwidth), but only if the model fits in VRAM, which can be tricky. Large MoE LLMs will get their active shard loaded in VRAM, with the rest sitting in system RAM. This works well up to a point; if the active shard is too big, it will get quite slow.
